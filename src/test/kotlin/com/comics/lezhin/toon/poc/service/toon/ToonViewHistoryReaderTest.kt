...
class ToonViewHistoryReaderTest {
    ...
    fun `toonId에 해당하는 조회 이력이 존재하면 리스트로 반환할 수 있다`() {
        ...
        `when`(toonViewHistoryRepository.findAllByToonId(id)).thenReturn(histories)

        val actual = sut.findToonViewHistoryListBy(id = id)

        assertEquals(2, actual?.size)
        verify(toonViewHistoryRepository, times(1)).findAllByToonId(id = id)
    }

    @Test
    fun `조회 이력이 없으면 빈 리스트를 반환할 수 있다`() {
        val id = 999L
        `when`(toonViewHistoryRepository.findAllByToonId(id = id)).thenReturn(emptyList())
